...
//          FileSystemEntry.elementWidth = maxElementWidth;
    ...
    try { // finally requestRepaintGPU()
      ...
      if (action == MotionEvent.ACTION_DOWN) {
        ...
        if (!touchMovement) {
          ...
          if (masterRoot.depth(touchEntry) > (int) touchDepth + 1) {
            return;
          }
  ...
  public void replaceRootKeepCursor(final FileSystemSuperRoot newRoot) {
    view.runInRenderThread(() -> {
      ...
      if (targetViewTop > newTop) {
        targetViewTop = newTop;
      }
      if (targetViewBottom < newBottom) {
        targetViewBottom = newBottom;
      }
      animationDuration = 300;
  ...
  private boolean preDraw() {
    ...
    if (curr > animationStartTime + animationDuration) {
      ...
      double f = interpolator
          .getInterpolation((curr - animationStartTime) / (float) animationDuration);
...
//        view.requestRepaint();
  ...
  private void paintSlowGPU(final RenderingThread rt,
    ...
    masterRoot
        .paintGPU(rt, bounds2, cursor, viewTop, viewDepth, yscale, screenHeight, numSpecialEntries);
  }
...
//    }
  ...
  private void deleteDeletingEntry() {
    ...
    if (freeSpace != null) {
      freeSpace
          .setSizeInBlocks(freeSpace.getSizeInBlocks() + deletingEntryBlocks, displayBlockSize);
      masterRoot
          .setSizeInBlocks(masterRoot.getSizeInBlocks() + deletingEntryBlocks, displayBlockSize);
...
//    Log.d("diskusage", "newSize = + " + newSize);
  ...
  public final boolean onKeyDown(final int keyCode, final KeyEvent event) {
    ...
    try { // finally requestRepaintGPU()
      ...
      if (keyCode == KeyEvent.KEYCODE_DPAD_RIGHT) {
        ...
        float requiredDepth =
            cursor.depth + 1 + (cursor.position.children == null ? 0 : 1) - maxLevels;
  ...
  private long getFreeSpaceZoom() {
    ...
    if (freeSpace == null) {
      return masterRoot.getSizeForRendering();
    }
